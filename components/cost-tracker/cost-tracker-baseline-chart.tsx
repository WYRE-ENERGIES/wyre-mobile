  ...
  const { barData, maxValue } = useMemo(() => {
    ...
    const barData = points.map((point, index) => ({
      labelComponent: labels[index]
        ...
        : undefined,
      stacks: [
        { value: point.used, color: WyreColors.purple },
        { value: Math.max(point.forecast - point.used, 0), color: '#E6E9EE' },
      ],
    }));
  ...
  if (!points.length) {
    return (
      <View style={styles.empty}>
        <Text style={styles.emptyText}>No baseline consumption data yet.</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container} onLayout={onLayout}>
      <View style={styles.legend}>
        ...
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: '#E6E9EE' }]} />
          <Text style={styles.legendText}>Remaining forecast</Text>
        </View>
      </View>
