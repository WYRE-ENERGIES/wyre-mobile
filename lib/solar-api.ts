...
export async function fetchPvProductionChart(
  branchId: number,
  date: Date,
): Promise<SolarHourlyChart> {
  const { month, year } = getMonthYear(date);
  const day = date.getDate();
  const response = await APIService.get(
    `solar/${branchId}/pv-production-hourly-plot/?month=${month}&year=${year}&day=${day}`,
  );
  return unwrapData<SolarHourlyChart>(response);
}
...
export async function fetchBatteryChart(
  ...
  const day = date.getDate();
  const response = await APIService.get(
    `solar/${branchId}/battery-backup-hourly-plot/?month=${month}&year=${year}&day=${day}`,
  );
  return unwrapData<SolarHourlyChart>(response);
}
